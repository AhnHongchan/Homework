...
while True:
    ...
    # targetBall_x, targetBall_y가 받는 ball이 들어갔거나, 존재하지 않는 경우 그 다음 ball의 정보를 받아온다.
    # 주어진 문제에서 balls[1], balls[3]. balls[5]만 타겟이므로 순서대로 넣으면 된다.
    if order == 1:
    ...
    # 완전한 일직선으로 가는 걸 막기 위해 width에 살짝 보정
    ...
    # 근거리일 때 power 자체를 좀 더 세게 함
    power = 20 + distance * 0.6
